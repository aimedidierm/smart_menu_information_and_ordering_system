...
if(isset($_POST['save'])){
    ...
    $sql ="INSERT INTO manager (email,names,phone,address,password) VALUES (?,?,?,?,?)";
    $stm = $db->prepare($sql);
    if ($stm->execute(array($email,$names,$phone,$address,$password))) {
        print "<script>alert('Manager added');window.location.assign('managers.php')</script>";

    } else{
        echo "<script>alert('Error! try again');window.location.assign('manager.php')</script>";
}
}
?>
<!DOCTYPE html>
...
<body>
  <div class="container-scroller">
                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive pt-3">
                    ...
                    if ($stm->execute(array($sid))) {
                        print "<script>alert('Manager deleted');window.location.assign('manager.php')</script>";
            
                    } else {
                        print "<script>alert('Delete fail');window.location.assign('manager.php')</script>";
                    }
                    }
                    ?>
                    </table>
    ...
    <div class="card shadow mb-6">
        ...
        <button type="submit" class="btn btn-facebook btn-block" name="save">Add Manager</button>
        </div>
        </form>
    </div>
          </div>
