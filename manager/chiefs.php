...
if(isset($_POST['save'])){
    ...
    $sql ="INSERT INTO chief (email,names,phone,address,password) VALUES (?,?,?,?,?)";
    $stm = $db->prepare($sql);
    if ($stm->execute(array($email,$names,$phone,$address,$password))) {
        print "<script>alert('Chief added');window.location.assign('chiefs.php')</script>";

    } else{
        echo "<script>alert('Error! try again');window.location.assign('chiefs.php')</script>";
}
}
?>
<!DOCTYPE html>
  ...
  <div class="container-scroller">
                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive pt-3">
                    ...
                    if ($stm->execute(array($sid))) {
                        ...
                        print "<script>alert('Delete fail');window.location.assign('chiefs.php')</script>";
                    }
                    }
                    ?>
                    </table>
    ...
    <div class="card shadow mb-6">
        ...
        <div class="card-header py-6">
            <h6 class="m-0 font-weight-bold text-primary">Add chief</h6>
        </div>
        <div class="card-body">
